import { Fragment, type Node, type Schema, Slice } from "../model";
import { type Mappable, StepMap } from "./editor.transform.map";
import { Step, StepResult } from "./editor.transform.step";
...
export class DocAttrStep extends Step {
  ...
  apply(doc: Node) {
    const attrs = Object.create(null);
    for (const name in doc.attrs) attrs[name] = doc.attrs[name];
    attrs[this.attr] = this.value;
