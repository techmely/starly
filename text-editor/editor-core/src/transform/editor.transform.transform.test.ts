...
import { Transform, findWrapping, liftTarget } from "techmely-editor-transform";
...
describe("Transform", () => {
  describe("addMark", () => {
    ...
    it("does not remove non-excluded marks of the same type", () => {
      ...
      const tr = new Transform(
        schema.node("doc", null, schema.text("hi", [schema.mark("comment", { id: 10 })])),
      );
    ...
    it("can remove multiple excluded marks", () => {
      ...
      const tr = new Transform(
        schema.node("doc", null, schema.text("hi", [schema.mark("small1"), schema.mark("small2")])),
      );
      ist(tr.doc.firstChild!.marks.length, 2);
    ...
    it("can remove more than one mark of the same type from a block", () => {
      ...
      const tr = new Transform(
        schema.node(
          "doc",
          null,
          schema.text("hi", [schema.mark("comment", { id: 1 }), schema.mark("comment", { id: 2 })]),
        ),
      );
      ist(tr.doc.firstChild!.marks.length, 2);
  ...
  describe("insert", () => {
    ...
    it("can insert a break", () =>
      ins(doc(p("hello<a>there")), schema.node("hard_break"), doc(p("hello", br(), "<a>there"))));
    ...
    it("can insert two block nodes", () =>
      ins(
        doc(p("one"), "<a>", p("two<2>")),
        [schema.node("paragraph", null, [schema.text("hi")]), schema.node("horizontal_rule")],
  ...
  describe("delete", () => {
    ...
    it("preserves content constraints", () =>
      del(doc(blockquote("<a>", p("hi"), "<b>"), p("x")), doc(blockquote(p()), p("x"))));
    ...
    it("doesn't join incompatible nodes", () =>
      del(doc(pre("fo<a>o"), p("b<b>ar", img())), doc(pre("fo"), p("ar", img()))));

    it("doesn't join when marks are incompatible", () =>
      del(doc(pre("fo<a>o"), p(em("b<b>ar"))), doc(pre("fo"), p(em("ar")))));
  });
  ...
  describe("join", () => {
    ...
    it("can join compatible blocks", () =>
      join(doc(h1("foo"), "<a>", p("bar")), doc(h1("foobar"))));

    it("can join nested blocks", () =>
      join(
        doc(
          blockquote(blockquote(p("a"), p("b<before>")), "<a>", blockquote(p("c"), p("d<after>"))),
        ),
  ...
  describe("split", () => {
    function split(
      ...
      else testTransform(new Transform(doc).split(tag(doc, "a"), depth, typesAfter), expect);
    }
    ...
    it("can split at end", () =>
      split(doc(blockquote(p("hi<a>"))), doc(blockquote(p("hi"), p("<a>")))));

    it("can split at start", () =>
      split(doc(blockquote(p("<a>hi"))), doc(blockquote(p(), p("<a>hi")))));
    ...
    it("respects the type param", () =>
      split(doc(h1("hell<a>o!")), doc(h1("hell"), p("<a>o!")), undefined, [
  ...
  describe("lift", () => {
    function lift(doc: Node, expect: Node) {
      const range = doc
        .resolve(tag(doc, "a"))
        .blockRange(doc.resolve(tag$(doc, "b") || tag(doc, "a")));
    ...
    it("can lift a single child", () =>
      lift(doc(blockquote(p("<a>t<in>wo"))), doc(p("<a>t<in>wo"))));
    ...
    it("can lift from a nested node", () =>
      lift(
        doc(
          blockquote(
            blockquote(p("<1>one"), p("<a>two"), p("<3>three"), p("<b>four"), p("<5>five")),
          ),
        ),
        doc(
          blockquote(
    ...
    it("can lift from the end of a list", () =>
      lift(doc(ul(li(p("a")), li(p("b<a>")), "<1>")), doc(ul(li(p("a"))), p("b<a>"), "<1>")));
  });
  ...
  describe("wrap", () => {
    function wrap(doc: Node, expect: Node, type: string, attrs?: Attrs) {
      const range = doc
        ...
        .blockRange(doc.resolve(tag$(doc, "b") || tag(doc, "a")));
      testTransform(
        new Transform(doc).wrap(range!, findWrapping(range!, schema.nodes[type], attrs)!),
        expect,
      );
    }
    ...
    it("can wrap in a list", () =>
      wrap(doc(p("<a>one"), p("<b>two")), doc(ol(li(p("<a>one"), p("<b>two")))), "ordered_list"));

    it("can wrap in a nested list", () =>
      wrap(
        doc(ol(li(p("<1>one")), li(p("..."), p("<a>two"), p("<b>three")), li(p("<4>four")))),
        doc(
          ol(li(p("<1>one")), li(p("..."), ol(li(p("<a>two"), p("<b>three")))), li(p("<4>four"))),
        ),
        "ordered_list",
      ));
  ...
  describe("setBlockType", () => {
    ...
    it("can change a single textblock", () =>
      type(doc(p("am<a> i")), doc(h2("am i")), "heading", { level: 2 }));
    ...
    it("clears markup when necessary", () =>
      type(doc(p("hello<a> ", em("world"))), doc(pre("hello world")), "code_block"));

    it("removes non-allowed nodes", () =>
      type(doc(p("<a>one", img(), "two", img(), "three")), doc(pre("onetwothree")), "code_block"));

    it("removes newlines in non-code", () =>
      type(doc(pre("<a>one\ntwo\nthree")), doc(p("one two three")), "paragraph"));

    it("only clears markup when needed", () =>
      type(doc(p("hello<a> ", em("world"))), doc(h1("hello<a> ", em("world"))), "heading", {
        level: 1,
      }));

    it("works after another step", () => {
      const d = doc(p("f<x>oob<y>ar"), p("baz<a>"));
      const tr = new Transform(d).delete(tag(d, "x"), tag(d, "y")),
        pos = tr.mapping.map(tag(d, "a"));
  ...
  describe("setNodeMarkup", () => {
    function markup(doc: Node, expect: Node, type: string, attrs?: Attrs) {
      testTransform(
        new Transform(doc).setNodeMarkup(tag(doc, "a"), schema.nodes[type], attrs),
        expect,
      );
    }

    it("can change a textblock", () =>
      markup(doc("<a>", p("foo")), doc(h1("foo")), "heading", { level: 1 }));
  ...
  describe("replace", () => {
    function repl(doc: Node, source: Node | Slice | null, expect: Node) {
      ...
      testTransform(
        new Transform(doc).replace(tag(doc, "a"), tag$(doc, "b") || tag(doc, "a"), slice),
        expect,
      );
    }
    ...
    it("can join blocks", () =>
      repl(doc(p("hell<a>o"), p("y<b>ou")), null, doc(p("hell<a><b>ou"))));
    ...
    it("can overwrite text", () =>
      repl(doc(p("hell<a>o y<b>ou")), doc(p("<a>i k<b>")), doc(p("hell<a>i k<b>ou"))));

    it("can insert text", () =>
      repl(doc(p("hell<a><b>o")), doc(p("<a>i k<b>")), doc(p("helli k<a><b>o"))));
    ...
    it("can insert while joining textblocks", () =>
      repl(doc(h1("he<a>llo"), p("arg<b>!")), doc(p("1<a>2<b>3")), doc(h1("he2!"))));
    ...
    it("merges blocks across deleted content", () =>
      repl(doc(p("a<a>"), p("b"), p("<b>c")), null, doc(p("a<a><b>c"))));

    it("can merge text down from nested nodes", () =>
      repl(doc(h1("wo<a>ah"), blockquote(p("ah<b>ha"))), null, doc(h1("wo<a><b>ha"))));
    ...
    it("can join different blocks", () =>
      repl(doc(h1("hell<a>o"), p("by<b>e")), null, doc(h1("helle"))));
    ...
    it("can insert into an empty block", () =>
      repl(doc(p("a"), p("<a>"), p("b")), doc(p("x<a>y<b>z")), doc(p("a"), p("y<a>"), p("b"))));
    ...
    it("can delete the whole document", () =>
      repl(doc("<a>", h1("hi"), p("you"), "<b>"), null, doc(p())));
    ...
    it("drops an empty node at the start of the slice", () =>
      repl(doc(p("<a>x")), doc(blockquote(p("hi"), "<a>"), p("b<b>")), doc(p(), p("bx"))));
    ...
    it("does nothing when given an unfittable slice", () =>
      repl(p("<a>x"), new Slice(Fragment.from([blockquote(), hr()]), 0, 0), p("x")));
    ...
    const ms = new Schema({
      nodes: schema.spec.nodes.update(
        "doc",
        Object.assign({}, schema.spec.nodes.get("doc"), { marks: "_" }),
      ),
    ...
    it("preserves marks on open slice block nodes", () => {
      ...
      tr.replace(
        ...
        3,
        ms
          .node("doc", null, [ms.node("paragraph", null, [ms.text("b")], [ms.mark("em")])])
          .slice(1, 3),
      );
    ...
    it("keeps isolating nodes together", () => {
      ...
      const doc = s.node("doc", null, [s.node("paragraph", null, [s.text("one")])]);
      const iso = Fragment.from(s.node("iso", null, [s.node("paragraph", null, s.text("two"))]));
      ist(
        new Transform(doc).replace(2, 3, new Slice(iso, 2, 0)).doc,
        s.node("doc", null, [
      ...
      ist(
        new Transform(doc).replace(2, 3, new Slice(iso, 2, 2)).doc,
        s.node("doc", null, [s.node("paragraph", null, [s.text("otwoe")])]),
        eq,
      );
    });
  });
  ...
  describe("replaceRange", () => {
    function repl(doc: Node, source: Node, expect: Node) {
      ...
      testTransform(
        new Transform(doc).replaceRange(tag(doc, "a"), tag$(doc, "b") || tag(doc, "a"), slice),
        expect,
      );
    }
    ...
    it("keeps defining context when it doesn't matches the parent markup", () => {
      ...
      const { b1, b2, b3, b4, b5, b6, p, doc } = builders(s, {
    ...
    it("drops defining context when it matches the parent structure", () =>
      repl(doc(blockquote(p("<a>"))), doc(blockquote(p("<a>one<b>"))), doc(blockquote(p("one")))));
  ...
  describe("replaceRangeWith", () => {
    function repl(doc: Node, node: Node, expect: Node) {
      testTransform(
        new Transform(doc).replaceRangeWith(tag(doc, "a"), tag$(doc, "b") || tag(doc, "a"), node),
        expect,
      );
    }
  ...
  describe("deleteRange", () => {
    function del(doc: Node, expect: Node) {
      testTransform(
        new Transform(doc).deleteRange(tag(doc, "a"), tag$(doc, "b") || tag(doc, "a")),
        expect,
      );
    }
    ...
    it("expands to cover the whole document", () =>
      del(doc(h1("<a>foo"), p("bar"), blockquote(p("baz<b>"))), doc(p())));

    it("doesn't expand beyond same-depth textblocks", () =>
      del(doc(h1("<a>foo"), p("bar"), p("baz<b>")), doc(h1())));

    it("deletes the open token when deleting from start to past end of block", () =>
      del(doc(h1("<a>foo"), p("b<b>ar")), doc(p("ar"))));
  ...
  describe("setDocAttribute", () => {
    const schema = new Schema({
      nodes: { doc: { content: "text*", attrs: { meta: { default: null } } }, text: {} },
    });

    const { doc } = builders(schema, {
      doc: { nodeType: "doc" },
    });
