import ist from "ist";
import type { MarkSpec, Node, NodeSpec, Schema } from "techmely-editor-model";
import { eq } from "techmely-editor-test-builder";
import { Mapping, Step, Transform } from "techmely-editor-transform";

function invert(transform: Transform) {
  const out = new Transform(transform.doc);
...
function testMapping(mapping: Mapping, pos: number, newPos: number) {
  ...
  const remap = new Mapping(mapping.maps.map((m) => m.invert()));
...
function testStepJSON(tr: Transform) {
  const newTR = new Transform(tr.before);
...
export function testTransform(tr: Transform, expect: Node) {
  ...
  for (const tag in (expect as any).tag)
...
function outputTransform(tr: Transform, expected: Node) {
  if (output && tr.steps.length) {
    ...
    for (const tag in (expected as any).tag)
...
function storeSchema(schema: Schema) {
  const known = output!.schemas.indexOf(schema);
