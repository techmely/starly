import ist from "ist";
import { type Node, Schema, Slice } from "techmely-editor-model";
import { eq, schema as baseSchema } from "techmely-editor-test-builder";
import { Transform, canSplit, findWrapping, liftTarget } from "techmely-editor-transform";
...
function n(name: string, ...content: Node[]) {
  return schema.nodes[name].create(null, content);
}
function t(str: string, em = false) {
...
describe("canSplit", () => {
  function yes(pos: number, depth?: number, after?: string) {
    return () =>
      ist(canSplit(doc, pos, depth, after === null ? undefined : [{ type: schema.nodes[after] }]));
  }
  ...
  it("doesn't return true when the split-off content doesn't fit in the given node type", () => {
    const s = new Schema({
      nodes: schema.spec.nodes
        .addBefore("heading", "title", { content: "text*" })
...
describe("findWrapping", () => {
  function yes(pos: number, end: number, type: string) {
    return () => {
      ...
      ist(r && findWrapping(r, schema.nodes[type]));
    };
  }
...
describe("Transform", () => {
  describe("replace", () => {
    ...
    it(
      "automatically adds a heading to a section",
      repl(
    ...
    it(
      "adds necessary nodes to the left",
      repl(
        n("doc", n("sect", n("head", t("foo")), n("para", t("bar")))),
        1,
        3,
    ...
    it(
      "adds necessary nodes to a parent node",
      repl(
        ...
        n(
          "doc",
          n("sect", n("head"), n("figure", n("caption"), n("figureimage")), n("para", t("hi"))),
        ),
      ),
    );
  });
