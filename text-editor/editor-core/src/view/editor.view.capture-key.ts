import { AllSelection, NodeSelection, TextSelection } from "../state/editor.model.selection";
import type { EditorState } from "../state/editor.model.state";
import * as browser from "./browser";
import { domIndex, hasBlockDesc, selectionCollapsed } from "./dom";
...
function selectHorizontally(view: EditorView, dir: number, mods: string) {
  ...
  if (sel instanceof TextSelection) {
    if (mods.indexOf("s") > -1) {
      ...
      const $newHead = view.state.doc.resolve($head.pos + node.nodeSize * (dir < 0 ? -1 : 1));
...
function skipIgnoredNodesBefore(view: EditorView) {
  ...
  // node if possible. Issue techmely-editor/techmely-editor#832.
  if (
    browser.gecko &&
    node.nodeType === 1 &&
    offset < nodeLen(node) &&
...
function textNodeBefore(node: Node | null, offset: number): Text | undefined {
  ...
  while (node && offset) {
    const next = node.childNodes[offset - 1];
    if (next.nodeType === 3) return next as Text;
    if (next.nodeType === 1 && (next as HTMLElement).contentEditable === "false") break;
    node = next;
...
function stopNativeHorizontalDelete(view: EditorView, dir: number) {
  ...
  const nextNode = !$head.textOffset && (dir < 0 ? $head.nodeBefore : $head.nodeAfter);
...
export function captureKeyDown(view: EditorView, event: KeyboardEvent) {
  ...
  if (code === 8 || (browser.mac && code === 72 && mods === "c")) {
    ...
    const dir =
      code === 37 ? (findDirection(view, view.state.selection.from) === "ltr" ? -1 : 1) : -1;
    return selectHorizontally(view, dir, mods) || skipIgnoredNodes(view, dir);
  } else if (code === 39 || (browser.mac && code === 70 && mods === "c")) {
    // Right arrow, Ctrl-f on Mac
    const dir =
      code === 39 ? (findDirection(view, view.state.selection.from) === "ltr" ? 1 : -1) : 1;
    return selectHorizontally(view, dir, mods) || skipIgnoredNodes(view, dir);
