import type { Mark, Node } from "techmely-editor-model";
import type { Mappable, Mapping } from "techmely-editor-transform";
...
export class InlineType implements DecorationType {
  ...
  map(mapping: Mappable, span: Decoration, offset: number, oldOffset: number): Decoration | null {
    const from = mapping.map(span.from + oldOffset, this.spec.inclusiveStart ? -1 : 1) - offset;
    const to = mapping.map(span.to + oldOffset, this.spec.inclusiveEnd ? 1 : -1) - offset;
...
export class Decoration {
  ...
  static widget(
    ...
    spec?: {
      ...
      /// widget are ignored, and don't cause techmely-editor to try and
      /// re-sync the selection with its selection state.
      ignoreSelection?: boolean;

      /// When comparing decorations of this type (in order to decide
      /// whether it needs to be redrawn), techmely-editor will by default
...
export class DecorationSet implements DecorationSource {
  ...
  private addInner(doc: Node, decorations: Decoration[], offset: number) {
    ...
    const local = moveSpans(childIndex ? withoutNulls(decorations) : decorations, -offset);
  ...
  private removeInner(decorations: (Decoration | null)[], offset: number) {
    ...
    for (let i = 0; i < children.length; i += 3) {
      ...
      const removed = (children[i + 2] as DecorationSet).removeInner(found, from + 1);
      if (removed !== empty) {
...
function buildTree(
  ...
  node.forEach((childNode, localStart) => {
    ...
    if (found) {
      hasNulls = true;
      const subtree = buildTree(found, childNode, offset + localStart + 1, options);
      if (subtree !== empty) children.push(localStart, localStart + childNode.nodeSize, subtree);
    }
  });
  const locals = moveSpans(hasNulls ? withoutNulls(spans) : spans, -offset).sort(byPos);
