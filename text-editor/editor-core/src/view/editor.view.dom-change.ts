...
import { Selection, TextSelection } from "techmely-editor-state";
...
function parseBetween(view: EditorView, from_: number, to_: number) {
  ...
  const parser = view.someProp("domParser") || DOMParser.fromSchema(view.state.schema);
...
export function readDOMChange(
  ...
  // the newly created block. And then, because techmely-editor cleans up
...
function findDiff(
  ...
  if (endA < start && a.size < b.size) {
    const move = preferredPos <= start && preferredPos >= endA ? start - preferredPos : 0;
    start -= move;
    ...
    const move = preferredPos <= start && preferredPos >= endB ? start - preferredPos : 0;
    start -= move;
