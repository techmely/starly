import ist from "ist";
import { Fragment, type Node, NodeRange, Schema, Slice } from "techmely-editor-model";
import { blockquote, builders, doc, h1, li, p, schema, ul } from "techmely-editor-test-builder";
import {
  ReplaceAroundStep,
  ReplaceStep,
  StepMap,
  Transform,
  liftTarget,
} from "techmely-editor-transform";
...
function make(d: DecoSpec) {
  ...
  if ((d as any).pos !== null)
    return Decoration.widget((d as { pos: number }).pos, widget, d as any);
  if ((d as any).node)
    return Decoration.node((d as any).from, (d as any).to, (d as any).attrs || {}, d);
...
function str(set: DecorationSet) {
  ...
  for (let i = 0; i < set.children.length; i += 3)
    s +=
      ...
      set.children[i] +
      ": " +
      str(set.children[i + 2] as DecorationSet);
  return s + "]";
}
...
function buildAdd(doc: Node, ...decorations: (DecoSpec | DecoSpec[])[]) {
  ...
  return build(doc, ...(decorations as DecoSpec[])).add(
    doc,
    Array.isArray(toAdd) ? toAdd.map(make) : [make(toAdd)],
  );
}

function buildRem(doc: Node, ...decorations: (DecoSpec | DecoSpec[])[]) {
  const toAdd = decorations.pop()!;
  return build(doc, ...(decorations as DecoSpec[])).remove(
    Array.isArray(toAdd) ? toAdd.map(make) : [make(toAdd)],
  );
}
...
describe("DecorationSet", () => {
  ...
  describe("find", () => {
    ...
    it("can filter by predicate", () => {
      const set = build(
        doc(blockquote(blockquote(p("a")))),
        { from: 3, to: 4, name: "X" },
        { from: 3, to: 4, name: "Y" },
      );
      ist(
        set
          .find(undefined, undefined, (x) => x.name === "Y")
          .map((d) => d.spec.name)
          .join(),
        "Y",
      );
  ...
  describe("map", () => {
    ...
    it("can map node decorations", () => {
      const { set } = buildMap(
        doc(blockquote(p("a"), p("b"))),
        { from: 4, to: 7, node: true },
        (tr) => tr.delete(1, 4),
      );
      ist(str(set), "[0: [0-3]]");
    });
    ...
    it("can map inside node decorations", () => {
      const { set } = buildMap(
        doc(blockquote(p("a"), p("b"))),
        { from: 4, to: 7, node: true },
        (tr) => tr.replaceWith(5, 5, schema.text("c")),
      );
      ist(str(set), "[0: [3-7]]");
    });

    it("removes partially overwritten node decorations", () => {
      const { set } = buildMap(doc(p("a"), p("b")), { from: 0, to: 3, node: true }, (tr) =>
        tr.delete(2, 4),
      );
      ist(str(set), "[]");
    });
    ...
    it("removes exactly overwritten node decorations", () => {
      const { set } = buildMap(doc(p("a"), p("b")), { from: 0, to: 3, node: true }, (tr) =>
        tr.replaceWith(0, 3, schema.nodes.horizontal_rule.create()),
      );
      ist(str(set), "[]");
    });

    it("isn't inclusive by default", () => {
      const { set } = buildMap(doc(p("foo")), { from: 2, to: 3 }, (tr) =>
        tr.replaceWith(2, 2, schema.text(".")).replaceWith(4, 4, schema.text("?")),
      );
      ist(str(set), "[0: [2-3]]");
    });

    it("understands unclusiveLeft", () => {
      const { set } = buildMap(doc(p("foo")), { from: 2, to: 3, inclusiveStart: true }, (tr) =>
        tr.replaceWith(2, 2, schema.text(".")).replaceWith(4, 4, schema.text("?")),
      );
      ist(str(set), "[0: [1-3]]");
    });
    ...
    it("understands unclusiveRight", () => {
      const { set } = buildMap(doc(p("foo")), { from: 2, to: 3, inclusiveEnd: true }, (tr) =>
        tr.replaceWith(2, 2, schema.text(".")).replaceWith(4, 4, schema.text("?")),
      );
      ist(str(set), "[0: [2-4]]");
    });
    ...
    it("preserves subtrees not touched by mapping", () => {
      const { oldSet, set } = buildMap(
        doc(p("foo"), blockquote(p("bar"), p("baz"))),
        { from: 2, to: 3 },
        { from: 8, to: 9 },
        { from: 13, to: 14 },
        (tr) => tr.delete(8, 9),
    ...
    it("rebuilds when a node is split", () => {
      const { set } = buildMap(doc(p("foobar")), { from: 2, to: 3 }, { from: 5, to: 6 }, (tr) =>
        tr.split(4),
      );
    ...
    it("correctly rebuilds a deep structure", () => {
      const { oldSet, set } = buildMap(
        doc(blockquote(p("foo")), blockquote(blockquote(p("bar")))),
        { from: 3, to: 4 },
        { from: 11, to: 12 },
        (tr) => tr.join(7),
      );
    ...
    it("calls onRemove when dropping decorations", () => {
      ...
      const tr = new Transform(d).delete(2, 6),
        dropped: string[] = [];
      set.map(tr.mapping, tr.doc, { onRemove: (o) => dropped.push(o.name) });
      ist(JSON.stringify(dropped), '["a"]');
      const tr2 = new Transform(d).delete(0, d.content.size),
        dropped2: string[] = [];
    ...
    it("maps inline decorations through ranges with > 3 elements", () => {
      ...
      class MyStep extends ReplaceStep {
        ...
        getMap() {
          return new StepMap(this.ranges);
        }
        merge(other: MyStep) {
          return null;
        }
      ...
      di.resolve(di.tag.start)
        .node(2)
        .forEach((node, offset, index) => {
      ...
      const tr = new Transform(di).step(
        new MyStep(di.tag.start - 2, di.tag.end + 2, slice, false, ranges),
      );

      const startSet = build(
        di,
        { from: di.tag.s1, to: di.tag.e1 },
        { from: di.tag.s2, to: di.tag.e2 },
      );
      const expectedSet = build(
        df,
        { from: df.tag.s1, to: df.tag.e1 },
        { from: df.tag.s2, to: df.tag.e2 },
      );
      ...
      const handMappedSet = DecorationSet.create(
        df,
        startSet.find().map((deco) =>
          make({
    ...
    it("correctly offsets a deep structure", () => {
      ...
      let decorations: DecoSpec[] = [],
        i = 0;
      const tr = new Transform(d);
      d.descendants((node, pos) => {
        if (node.type.name === "paragraph") {
          if (i++ % 2 === 0) decorations.push({ pos: pos + 1 });
          else tr.replaceWith(tr.mapping.map(pos), tr.mapping.map(pos + node.nodeSize, -1), p());
        }
      });
      const oldSet = build(d, ...decorations),
        set = oldSet.map(tr.mapping, tr.doc);
    ...
    it("doesn't get confused by composite changes", () => {
      const d = doc(p("x"), p("a"), p("x"), blockquote(p("<a>b<b>")), p("x"));
      const tr = new Transform(d).delete(14, 17).delete(6, 9).delete(0, 3);
      const deco = DecorationSet.create(d, [Decoration.inline(d.tag.a, d.tag.b, {})]);
      const changed = deco.map(tr.mapping, tr.doc);
      ist(
        changed.find().map((d) => d.from + "-" + d.to),
        "5-6",
      );
    });
  });
  ...
  describe("add", () => {
    it("can add a local decoration", () => {
      ist(str(buildAdd(doc(p("foo"), p("bar")), { pos: 0 }, { pos: 5 })), "[0-0, 5-5]");
    });

    it("can add a decoration in a new child", () => {
      ist(str(buildAdd(doc(p("foo"), p("bar")), { pos: 0 }, { pos: 3 })), "[0-0, 0: [2-2]]");
    });

    it("can add a decoration to an existing child", () => {
      ist(str(buildAdd(doc(p("foo"), p("bar")), { pos: 1 }, { pos: 3 })), "[0: [0-0, 2-2]]");
    });

    it("can add a decoration beyond an existing child", () => {
      ist(str(buildAdd(doc(blockquote(p("foo"))), { pos: 1 }, { pos: 4 })), "[0: [0-0, 0: [2-2]]]");
    });

    it("can add multiple decorations", () => {
      ist(
        str(
          buildAdd(doc(p("foo"), p("bar")), { pos: 1 }, { from: 6, to: 9 }, [
            { from: 1, to: 4 },
            { pos: 7 },
            { pos: 8 },
          ]),
  ...
  describe("remove", () => {
    ...
    it("can delete multiple decorations", () => {
      const d1 = make({ pos: 2 }),
        d2 = make({ from: 2, to: 8 });
      ist(
        str(buildRem(doc(p("foo"), p("bar")), { pos: 1 }, { from: 6, to: 7 }, d1, d2, [d1, d2])),
        "[0: [0-0], 5: [0-1]]",
      );
    });
    ...
    it("compares by both position and type when removing", () => {
      const deco = DecorationSet.create(
        ...
        [
          [1, 2],
          [3, 4],
        ].map(([from, to]) => Decoration.inline(from, to, {})),
      );
