import { DOMSerializer, Fragment, Mark, Node, type ParseRule } from "techmely-editor-model";
...
import { type DOMNode, domIndex, isEquivalentPosition, nodeSize } from "./dom";
...
export interface NodeView {
  ...
  /// type is not a leaf node type. When this is present, techmely-editor
  ...
  /// its child nodes will be handled by techmely-editor.
  update?: (
    node: Node,
...
class WidgetViewDesc extends ViewDesc {
  constructor(
    ...
    if (typeof dom === "function")
      dom = dom(view, () => {
        if (!this) return pos;
        if (this.parent) return this.parent.posBeforeChild(this);
      });
    if (!widget.type.spec.raw) {
      ...
      (dom as HTMLElement).classList.add("techmely-editor-widget");
    }
...
export class NodeViewDesc extends ViewDesc {
  ...
  updateChildren(view: EditorView, pos: number) {
    let inline = this.node.inlineContent,
      off = pos;
    const composition = view.composing ? this.localCompositionInfo(view, pos) : null;
    const localComposition = composition && composition.pos > -1 ? composition : null;
    const compositionInChild = composition && composition.pos < 0;
    const updater = new ViewTreeUpdater(this, localComposition && localComposition.node, view);
    iterDeco(
      this.node,
      this.innerDeco,
      (widget, i, insideNode) => {
  ...
  localCompositionInfo(
    ...
    if (this.node.inlineContent) {
      ...
      const textPos = findTextInFragment(this.node.content, text, from - pos, to - pos);
  ...
  selectNode() {
    if (this.nodeDOM.nodeType === 1)
      (this.nodeDOM as HTMLElement).classList.add("techmely-editor-selectednode");
  ...
  deselectNode() {
    if (this.nodeDOM.nodeType === 1)
      (this.nodeDOM as HTMLElement).classList.remove("techmely-editor-selectednode");
    if (this.contentDOM || !this.node.type.spec.draggable)
...
export function docViewDesc(
  ...
  const docView = new NodeViewDesc(undefined, doc, outerDeco, innerDeco, dom, dom, dom, view, 0);
...
class ViewTreeUpdater {
  ...
  syncToMarks(marks: readonly Mark[], inline: boolean, view: EditorView) {
    ...
    while (depth < marks.length) {
      ...
      if (found > -1) {
        ...
        const markDesc = MarkViewDesc.create(this.top, marks[depth], inline, view);
  ...
  recreateWrapper(
    ...
    if (next.dirty || node.isAtom || !next.children.length || !next.node.content.eq(node.content))
      return null;
    const wrapper = NodeViewDesc.create(this.top, node, outerDeco, innerDeco, view, pos);
  ...
  addNode(
    ...
    const desc = NodeViewDesc.create(this.top, node, outerDeco, innerDeco, view, pos);
  ...
  placeWidget(widget: Decoration, view: EditorView, pos: number) {
    const next = this.index < this.top.children.length ? this.top.children[this.index] : null;
    if (
      next &&
      ...
      const desc = new WidgetViewDesc(this.top, widget, view, pos);
      this.top.children.splice(this.index++, 0, desc);
      this.changed = true;
    }
  }
  ...
  addHackNode(nodeName: string, parent: ViewDesc) {
    if (
      ...
      if (nodeName === "BR") dom.className = "techmely-editor-trailingBreak";
