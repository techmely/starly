...
export function parseFromClipboard(
  ...
  if (asText) {
    ...
    if (inCode)
      return text
        ...
        : Slice.empty;
    const parsed = view.someProp("clipboardTextParser", (f) => f(text, $context, plainText, view));
    if (parsed) {
      slice = parsed;
    } else {
  ...
  if (sliceData) {
    ...
    // HTML wasn't created by techmely-editor. Make sure top-level siblings are coherent
...
function addToSibling(
  ...
  if (depth < wrap.length && depth < lastWrap.length && wrap[depth] === lastWrap[depth]) {
    const inner = addToSibling(wrap, lastWrap, node, sibling.lastChild!, depth + 1);
...
function closeRight(node: Node, depth: number) {
  ...
  const fill = node.contentMatchAt(node.childCount).fillBefore(Fragment.empty, true)!;
