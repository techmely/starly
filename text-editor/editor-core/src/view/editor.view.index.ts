import * as browser from "./editor.view.browser";
import { Decoration, type DecorationSource, viewDecorations } from "./editor.view.decoration";
import {
  type DOMNode,
  type DOMSelection,
  type DOMSelectionRange,
  deepActiveElement,
} from "./editor.view.dom";
import { readDOMChange } from "./editor.view.dom-change";
import {
  coordsAtPos,
  endOfTextblock,
  focusPreventScroll,
  ...
  scrollRectIntoView,
  storeScrollPos,
} from "./editor.view.dom-coords";
import { DOMObserver, safariShadowSelectionRange } from "./editor.view.dom-observer";
import {
  Dragging,
  InputState,
  clearComposition,
  ...
  ensureListeners,
  initInput,
} from "./editor.view.input";
import { anchorInRightPlace, selectionToDOM, syncNodeSelection } from "./editor.view.selection";
import {
  type NodeView,
  type NodeViewDesc,
  type ViewDesc,
  docViewDesc,
} from "./editor.view.viewdesc";
...
export {
  ...
  DecorationAttrs,
  DecorationSource,
} from "./editor.view.decoration";
export { NodeView } from "./editor.view.viewdesc";

import type { DOMSerializer, Mark, ResolvedPos, Slice } from "../model";
import type { PluginView } from "../state/editor.model.plugin";
import { NodeSelection } from "../state/editor.model.selection";
import type { EditorState } from "../state/editor.model.state";
import type { Transaction } from "../state/editor.model.transaction";
// Exported for testing
import { parseFromClipboard, serializeForClipboard } from "./editor.view.clipboard";
import { endComposition } from "./editor.view.input";
/// @internal
...
export class EditorView {
  ...
  /// [space is eaten issue](https://github.com/techmely-editor/techmely-editor/issues/651)
  /// @internal
  ...
  setProps(props: Partial<DirectEditorProps>) {
    ...
    for (const name in this._props) (updated as any)[name] = (this._props as any)[name];
    updated.state = this.state;
    for (const name in props) (updated as any)[name] = (props as any)[name];
    this.update(updated);
  }
    ...
    const scroll =
      prev.plugins !== state.plugins && !prev.doc.eq(state.doc)
        ...
        : (state as any).scrollToSelection > (prev as any).scrollToSelection
          ? "to selection"
          : "preserve";
    const updateDoc = redraw || !this.docView.matchesNode(state.doc, outerDeco, innerDeco);
  ...
  private updateDraggedNode(dragging: Dragging, prev: EditorState) {
    ...
    if (this.state.doc.nodeAt(sel.from) === sel.node) {
      ...
      const movedPos = sel.from + (this.state.doc.content.size - prev.doc.content.size);
      const moved = movedPos > 0 && this.state.doc.nodeAt(movedPos);
...
function updateCursorWrapper(view: EditorView) {
  if (view.markCursor) {
    ...
    dom.className = "techmely-editor-separator";
...
function selectionContextChanged(sel1: Selection, sel2: Selection) {
  const depth = Math.min(sel1.$anchor.sharedDepth(sel1.head), sel2.$anchor.sharedDepth(sel2.head));
...
export interface EditorProps<P = any> {
  ...
  /// techmely-editor does of events fired on the editable DOM element.
  ...
  /// By default, the element will get a class `"techmely-editor"`, and
