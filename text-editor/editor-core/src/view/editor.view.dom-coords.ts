import type { EditorState } from "techmely-editor-state";
import * as browser from "./browser";
import { caretFromPoint, nodeSize, parentNode, textRange } from "./dom";
...
export function posAtCoords(view: EditorView, coords: { top: number; left: number }) {
  let doc = view.dom.ownerDocument,
    node: Node | undefined,
    offset = 0;
  const caret = caretFromPoint(doc, coords.left, coords.top);
  if (caret) ({ node, offset } = caret);

  let elt = ((view.root as any).elementFromPoint ? view.root : doc).elementFromPoint(
    coords.left,
    coords.top,
  ) as HTMLElement;
  let pos;
  ...
  if (node) {
    if (browser.gecko && node.nodeType === 1) {
      ...
      if (offset < node.childNodes.length) {
        ...
        if (
          next.nodeName === "IMG" &&
          (box = (next as HTMLElement).getBoundingClientRect()).right <= coords.left &&
...
export function coordsAtPos(view: EditorView, pos: number, side: number): Rect {
  ...
  if (node.nodeType === 3) {
    ...
    if (
      ...
      if (
        ...
        const rectBefore = singleRect(textRange(node as Text, offset - 1, offset - 1), -1);
        if (rectBefore.top === rect.top) {
          const rectAfter = singleRect(textRange(node as Text, offset, offset + 1), -1);
  ...
  return flattenV(
    singleRect(node.nodeType === 3 ? textRange(node as Text) : (node as HTMLElement), -side),
    side >= 0,
  );
}
...
function endOfTextblockHorizontal(
  ...
  return withFlushedState(view, state, () => {
    ...
    const parentDOM = $head.depth ? view.docView.domAfterPos($head.before()) : view.dom;
    const { focusNode: newNode, focusOffset: newOff } = view.domSelectionRange();
    const result =
