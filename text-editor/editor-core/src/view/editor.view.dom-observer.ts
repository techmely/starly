import { Selection } from "techmely-editor-state";
...
import { hasFocusAndSelection, selectionFromDOM, selectionToDOM } from "./selection";
...
export class DOMObserver {
  ...
  constructor(
    ...
    this.observer =
      ...
      new window.MutationObserver((mutations) => {
        ...
        // techmely-editor to miss the change (see #930)
        if (
          browser.ie &&
  ...
  ignoreSelectionChange(sel: DOMSelectionRange) {
    ...
    let ancestors: Set<Node> = new Set(),
      container: DOMNode | undefined;
    for (let scan: DOMNode | null = sel.focusNode; scan; scan = parentNode(scan))
      ancestors.add(scan);
  ...
  pendingRecords() {
    if (this.observer) for (const mut of this.observer.takeRecords()) this.queue.push(mut);
    return this.queue;
  }
  ...
  registerMutation(mut: MutationRecord, added: Node[]) {
    ...
    if (mut.type === "childList") {
      ...
      const fromOffset = prev && prev.parentNode === mut.target ? domIndex(prev) + 1 : 0;
...
function checkCSS(view: EditorView) {
  ...
  if (["normal", "nowrap", "pre-line"].indexOf(getComputedStyle(view.dom).whiteSpace) !== -1) {
    ...
    console["warn"](
      "techmely-editor expects the CSS white-space property to be set, preferably to 'pre-wrap'. It is recommended to load style/techmely-editor.css from the techmely-editor-view package.",
    );
    cssCheckWarned = true;
  }
}
...
export function safariShadowSelectionRange(view: EditorView): DOMSelectionRange {
  ...
  const currentAnchor = view.domAtPos(view.state.selection.anchor);
