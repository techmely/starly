import { type Node, Slice } from "techmely-editor-model";
import { NodeSelection, Selection, TextSelection } from "techmely-editor-state";
import { dropPoint } from "techmely-editor-transform";
...
editHandlers.keypress = (view, _event) => {
  ...
  if (!(sel instanceof TextSelection) || !sel.$from.sameParent(sel.$to)) {
    ...
    if (
      !/[\r\n]/.test(text) &&
      !view.someProp("handleTextInput", (f) => f(view, sel.$from.pos, sel.$to.pos, text))
    )
  ...
  up(event: MouseEvent) {
    ...
    if (this.allowDefault || !pos) {
      ...
      (this.flushed ||
        ...
        // thus doesn't get a reaction from techmely-editor. This
        // works around that.
        (browser.chrome &&
    ...
    if (
      ...
      if (
        ...
        for (
          ...
          const before = offset < 0 ? node.lastChild : node.childNodes[offset - 1];
          if (!before) break;
...
function capturePaste(view: EditorView, event: ClipboardEvent) {
  if (!view.dom.parentNode) return;
  const plainText = view.input.shiftKey || view.state.selection.$from.parent.type.spec.code;
...
export function doPaste(
  ...
  const slice = parseFromClipboard(view, text, html, preferPlain, view.state.selection.$from);
...
handlers.dragstart = (view, _event) => {
  ...
  const slice = (node || view.state.selection).content(),
    { dom, text } = serializeForClipboard(view, slice);
  event.dataTransfer.clearData();
  event.dataTransfer.setData(brokenClipboardAPI ? "Text" : "text/html", dom.innerHTML);
  // See https://github.com/techmely-editor/techmely-editor/issues/1156
...
editHandlers.drop = (view, _event) => {
  ...
  if (move) {
    const { node } = dragging as Dragging;
...
handlers.focus = (view) => {
  ...
  if (!view.focused) {
    view.domObserver.stop();
    view.dom.classList.add("techmely-editor-focused");
...
handlers.blur = (view, _event) => {
  ...
  if (view.focused) {
    view.domObserver.stop();
    view.dom.classList.remove("techmely-editor-focused");
    view.domObserver.start();
