...
import {
  ...
  type SelectionRange,
  TextSelection,
} from "../state/editor.model.selection";
import type { EditorState } from "../state/editor.model.state";
import type { Command, Transaction } from "../state/editor.model.transaction";
import {
  ReplaceAroundStep,
  ReplaceStep,
  canJoin,
...
import type { EditorView } from "../view/editor.view.index";
...
export const joinUp: Command = (state, dispatch) => {
  ...
  if (dispatch) {
    ...
    if (nodeSel)
      tr.setSelection(
        NodeSelection.create(tr.doc, point - state.doc.resolve(point).nodeBefore!.nodeSize),
      );
    dispatch(tr.scrollIntoView());
  }
  return true;
...
export function autoJoin(
  ...
  const canJoin = Array.isArray(isJoinable)
