...
import { NodeSelection, TextSelection } from "../state/editor.model.selection";
import { EditorState } from "../state/editor.model.state";
import type { Command } from "../state/editor.model.transaction";
import { blockquote, doc, em, eq, h1, hr, img, li, ol, p, pre, strong, ul } from "../test-utils";
import {
  autoJoin,
  createParagraphNear,
  deleteSelection,
...
describe("joinBackward", () => {
  ...
  it("can join out of a nested node", () =>
    apply(doc(p("hi"), blockquote(p("<a>there"))), joinBackward, doc(p("hi"), p("there"))));
  ...
  it("moves a block into a list item", () =>
    apply(doc(ul(li(p("hi"))), p("<a>there")), joinBackward, doc(ul(li(p("hi")), li(p("there"))))));
  ...
  it("joins list items", () =>
    apply(doc(ul(li(p("hi")), li(p("<a>there")))), joinBackward, doc(ul(li(p("hi"), p("there"))))));
  ...
  it("does nothing at start of doc", () => apply(doc(p("<a>foo")), joinBackward, null));
...
describe("joinTextblockBackward", () => {
  ...
  it("does nothing at start of doc", () => apply(doc(p("<a>foo")), joinTextblockBackward, null));
...
describe("deleteSelection", () => {
  ...
  it("moves selection before deleted node at end", () =>
    apply(doc(p("a"), "<a>", p("b")), deleteSelection, doc(p("a<a>"))));
});
...
describe("joinForward", () => {
  ...
  it("does nothing at the end of the document", () => apply(doc(p("foo<a>")), joinForward, null));
  ...
  it("pulls the next block into the current list item", () =>
    apply(doc(ul(li(p("a<a>")), li(p("b")))), joinForward, doc(ul(li(p("a"), p("b"))))));

  it("joins two blocks inside of a list item", () =>
    apply(doc(ul(li(p("a<a>"), p("b")))), joinForward, doc(ul(li(p("ab"))))));
  ...
  it("pulls the next block outside of a wrapping blockquote", () =>
    apply(doc(p("foo<a>"), blockquote(p("bar"))), joinForward, doc(p("foo"), p("bar"))));
  ...
  it("does nothing in a nested node at the end of the document", () =>
    apply(doc(ul(li(p("there<a>")))), joinForward, null));

  it("deletes a leaf node at the end of the document", () =>
    apply(doc(p("there<a>"), hr()), joinForward, doc(p("there"))));

  it("moves before it deletes a leaf node", () =>
    apply(doc(blockquote(p("there<a>")), hr()), joinForward, doc(blockquote(p("there"), hr()))));

  it("does nothing when it can't join", () =>
    apply(doc(p("foo<a>"), ul(li(p("bar"), ul(li(p("baz")))))), joinForward, null));
});
...
describe("joinTextblockForward", () => {
  ...
  it("does nothing at end of doc", () => apply(doc(p("foo<a>")), joinTextblockForward, null));
});
...
describe("selectNodeForward", () => {
  ...
  it("does nothing at end of document", () => apply(doc(p("foo<a>")), selectNodeForward, null));
});
...
describe("joinUp", () => {
  ...
  it("joins lists", () =>
    apply(doc(ul(li(p("foo"))), ul(li(p("<a>bar")))), joinUp, doc(ul(li(p("foo")), li(p("bar"))))));

  it("joins list items", () =>
    apply(doc(ul(li(p("foo")), li(p("<a>bar")))), joinUp, doc(ul(li(p("foo"), p("bar"))))));
...
describe("joinDown", () => {
  ...
  it("joins list items", () =>
    apply(doc(ul(li(p("<a>foo")), li(p("bar")))), joinDown, doc(ul(li(p("foo"), p("bar"))))));
...
describe("lift", () => {
  ...
  it("can lift out of a list", () => apply(doc(ul(li(p("<a>foo")))), lift, doc(p("foo"))));

  it("does nothing for a top-level block", () => apply(doc(p("<a>foo")), lift, null));

  it("lifts out of the innermost parent", () =>
    apply(doc(blockquote(ul(li(p("foo<a>"))))), lift, doc(blockquote(p("foo<a>")))));

  it("can lift a node selection", () =>
    apply(doc(blockquote("<a>", ul(li(p("foo"))))), lift, doc("<a>", ul(li(p("foo"))))));
...
describe("wrapIn", () => {
  ...
  it("can wrap a paragraph", () => apply(doc(p("fo<a>o")), wrap, doc(blockquote(p("foo")))));
  ...
  it("wraps an already wrapped node", () =>
    apply(doc(blockquote(p("fo<a>o"))), wrap, doc(blockquote(blockquote(p("foo"))))));

  it("can wrap a node selection", () =>
    apply(doc("<a>", ul(li(p("foo")))), wrap, doc(blockquote(ul(li(p("foo")))))));
});
...
describe("splitBlockAs", () => {
  it("splits to the appropriate type", () =>
    apply(
      doc(p("on<a>e")),
      splitBlockAs((n) => ({ type: n.type.schema.nodes.heading, attrs: { level: 1 } })),
      doc(p("on"), h1("<a>e")),
    ));
...
describe("liftEmptyBlock", () => {
  ...
  it("lifts the last child out of its parent", () =>
    apply(doc(blockquote(p("foo"), p("<a>"))), liftEmptyBlock, doc(blockquote(p("foo")), p())));
  ...
  it("lifts out of a list", () =>
    apply(doc(ul(li(p("hi")), li(p("<a>")))), liftEmptyBlock, doc(ul(li(p("hi"))), p())));
});
...
describe("setBlockType", () => {
  ...
  it("can make a heading into a paragraph", () => apply(doc(h1("fo<a>o")), setPara, doc(p("foo"))));

  it("preserves marks", () =>
    apply(doc(h1("fo<a>o", em("bar"))), setPara, doc(p("foo", em("bar")))));

  it("acts on node selections", () => apply(doc("<a>", h1("foo")), setPara, doc(p("foo"))));

  it("can make a block a code block", () => apply(doc(h1("fo<a>o")), setCode, doc(pre("foo"))));
...
describe("toggleMark", () => {
  ...
  const toggleEm2 = toggleMark(schema.marks.em, null, { removeWhenPresent: false });

  it("can add a mark", () => {
    apply(doc(p("one <a>two<b>")), toggleEm, doc(p("one ", em("two"))));
  });

  it("can stack marks", () => {
    apply(doc(p("one <a>tw", strong("o<b>"))), toggleEm, doc(p("one ", em("tw", strong("o")))));
  });

  it("can remove marks", () => {
    apply(doc(p(em("one <a>two<b>"))), toggleEm, doc(p(em("one "), "two")));
  });
  ...
  it("skips whitespace at selection ends when adding marks", () => {
    apply(doc(p("one<a> two  <b>three")), toggleEm, doc(p("one ", em("two"), "  three")));
  });

  it("doesn't skip whitespace-only selections", () => {
    apply(doc(p("one<a> <b>two")), toggleEm, doc(p("one", em(" "), "two")));
  });

  it("can add marks with remove-when-present off", () => {
    apply(doc(p("<a>", em("one"), " two<b>")), toggleEm2, doc(p(em("one two"))));
    apply(doc(p("<a>three<b>")), toggleEm2, doc(p(em("three"))));
  });

  it("can remove marks with remove-when-present off", () => {
    apply(doc(p(em("o<a>ne two<b>"))), toggleEm2, doc(p(em("o"), "ne two")));
  });

  it("can remove marks with trailing space when remove-when-present is off", () => {
    apply(doc(p(em("o<a>ne two"), "  <b>three")), toggleEm2, doc(p(em("o"), "ne two  three")));
  });
});

describe("selectTextblockStart and selectTextblockEnd", () => {
  it("can move the cursor when the selection is empty", () => {
    apply(doc(p("one <a>two")), selectTextblockStart, doc(p("<a>one two")));

    apply(doc(p("one <a>two")), selectTextblockEnd, doc(p("one two<a>")));
  });

  it("can move the cursor when the selection is not empty", () => {
    apply(doc(p("one <a>two<b>")), selectTextblockStart, doc(p("<a>one two")));

    apply(doc(p("one <a>two<b>")), selectTextblockEnd, doc(p("one two<a>")));
  });
