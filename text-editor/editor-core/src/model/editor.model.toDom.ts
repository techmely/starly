...
import type { Mark } from "./editor.model.mark";
import type { Node } from "./editor.model.node";
import type { MarkType, NodeType, Schema } from "./editor.model.schema";
...
/// A DOM serializer knows how to convert Techmely nodes and
...
export class DOMSerializer {
  ...
  static renderSpec(
    ...
    for (let i = start; i < (structure as readonly any[]).length; i++) {
      ...
      const { dom: inner, contentDOM: innerContent } = DOMSerializer.renderSpec(doc, child, xmlNS);
...
function gatherToDOM(obj: { [node: string]: NodeType | MarkType }) {
  const result: { [node: string]: (value: any, inline: boolean) => DOMOutputSpec } = {};
