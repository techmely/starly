...
import { findDiffEnd, findDiffStart } from "./editor.model.diff";
import type { Node, TextNode } from "./editor.model.node";
import type { Schema } from "./editor.model.schema";
...
export class Fragment {
  ...
  nodesBetween(from: number, to: number, f: NodeCallbackFn, nodeStart = 0, parent?: Node) {
  ...
  append(other: Fragment) {
    ...
    let i = 0;
    if (last?.isText && first && last.sameMarkup(first)) {
    ...
    throw new RangeError(
      `Can not convert ${nodes} to a Fragment${
        (nodes as any).nodesBetween
          ? " (looks like multiple versions of Techmely-model were loaded)"
          : ""
      }`,
    );
  }
