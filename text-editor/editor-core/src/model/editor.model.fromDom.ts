import type { DOMNode } from "../editor.types";
import type { ContentMatch } from "./editor.model.content";
import { Fragment } from "./editor.model.fragment";
import { Mark } from "./editor.model.mark";
import type { Node, TextNode } from "./editor.model.node";
import { Slice } from "./editor.model.replace";
import type { ResolvedPos } from "./editor.model.resolvedpos";
import type { Attrs, MarkType, NodeType, Schema } from "./editor.model.schema";
...
/// style as a Techmely node or mark.
...
/// Techmely document conforming to a given schema. Its behavior is
...
export class DOMParser {
  ...
  constructor(
    ...
    this.normalizeLists = !this.tags.some((r) => {
      if ((r.tag && !/^(ul|ol)\b/.test(r.tag)) || !r.node) return false;
  ...
  static schemaRules(schema: Schema) {
    ...
    function insert(rule: ParseRule) {
      ...
      for (; i < result.length; i++) {
        ...
        if (nextPriority && priority && nextPriority < priority) break;
      }
...
class NodeContext {
  ...
  constructor(
    ...
    this.match = match || (options & OPT_OPEN_LEFT ? null : type?.contentMatch || null);
  }
  ...
  findWrapping(node: Node) {
    if (!this.match) {
      ...
      if (fill) {
        ...
        const wrap = start.findWrapping(node.type);
        if (wrap) {
          this.match = start;
          return wrap;
        }
  ...
  finish(openEnd?: boolean): Node | Fragment {
    ...
    if (!openEnd && this.match) {
      const frag = this.match.fillBefore(Fragment.empty, true);
      if (frag) {
        content = content.append(frag);
      }
    }
  ...
  applyPending(nextType: NodeType) {
    for (let i = 0, pending = this.pendingMarks; i < pending.length; i++) {
      ...
      if (
        (this.type ? this.type.allowsMarkType(mark.type) : markMayApply(mark.type, nextType)) &&
  ...
  inlineContext(node: DOMNode) {
    ...
    return node.parentNode && !Object.hasOwn(blockTags, node.parentNode.nodeName.toLowerCase());
  }
}
...
class ParseContext {
  ...
  withStyleRules(dom: HTMLElement, f: () => void) {
    ...
    if (!style) return f();
    const marks = this.readStyles(parseStyles(style));
    if (!marks) return; // A style with ignore: true
    const [addMarks, removeMarks] = marks;
    const top = this.top;
      ...
      if (!(top.options & OPT_PRESERVE_WS)) {
        ...
        if (/^[ \t\r\n\u000c]/.test(value) && this.open === this.nodes.length - 1) {
          ...
          if (
            ...
            (nodeBefore.text && nodeBefore.isText && /[ \t\r\n\u000c]$/.test(nodeBefore.text))
          )
            value = value.slice(1);
        }
  ...
  addElement(dom: HTMLElement, matchAfter?: ParseRule) {
    ...
    if (Object.hasOwn(listTags, name) && this.parser.normalizeLists) normalizeList(dom);
    const ruleID = this.parser.matchTag(dom, this, matchAfter);
    const rule = this.options.ruleFromNode?.(dom) || ruleID;
    const isIgnore = rule ? rule.ignore : Object.hasOwn(ignoreTags, name);
    if (isIgnore) {
      this.findInside(dom);
      this.ignoreFallback(dom);
    } else if (!rule || rule.skip || rule.closeParent) {
      if (rule?.closeParent) this.open = Math.max(0, this.open - 1);
      ...
      if (Object.hasOwn(blockTags, name)) {
  ...
  leafFallback(dom: DOMNode) {
    if (dom.nodeName === "BR" && dom.ownerDocument && this.top.type && this.top.type.inlineContent)
  ...
  readStyles(styles: readonly string[]) {
    let add = Mark.none;
    let remove = Mark.none;
    for (let i = 0; i < styles.length; i += 2) {
      for (let after = undefined; ; ) {
        const rule = this.parser.matchStyle(styles[i], styles[i + 1], this, after);
        if (!rule) break;
        if (rule.ignore) return null;
        if (rule.clearMark) {
          const marks = this.top.pendingMarks.concat(this.top.activeMarks);
  ...
  addElementByRule(dom: HTMLElement, rule: ParseRule, continueAfter?: ParseRule) {
    ...
    if (nodeType?.isLeaf) {
      ...
      this.findInside(dom);
      const content = rule.getContent(dom, this.parser.schema);
      // biome-ignore lint/complexity/noForEach: Fragment class func
      content.forEach((node) => this.insertNode(node));
    } else {
      let contentDOM = dom;
      if (typeof rule.contentElement === "string") {
        const contentEl = dom.querySelector(rule.contentElement) as HTMLElement | null;
        if (contentEl) contentDOM = contentEl;
  ...
  addAll(parent: DOMNode, startIndex = 0, endIndex?: number) {
    let index = startIndex;
    for (
      let currentDom = startIndex ? parent.childNodes[startIndex] : parent.firstChild,
        endDom = endIndex === null || endIndex === undefined ? null : parent.childNodes[endIndex];
      currentDom !== endDom;
      currentDom = currentDom?.nextSibling || null, ++index
    ) {
      this.findAtPoint(parent, index);
      if (currentDom) {
        this.addDOM(currentDom);
      }
    }
    this.findAtPoint(parent, index);
  }
  ...
  enterInner(
    ...
    top.match = top?.match?.matchType(type) || null;
    let options = wsOptionsFor(type, preserveWS, top.options);
    if (top.options & OPT_OPEN_LEFT && top.content.length === 0) options |= OPT_OPEN_LEFT;
    this.nodes.push(
      new NodeContext(type, attrs, top.activeMarks, top.pendingMarks, solid, null, options),
    );
    this.open++;
  }
  ...
  findInText(textNode: Text) {
    if (this.find)
      for (let i = 0; i < this.find.length; i++) {
        if (this.find[i].node === textNode && textNode.nodeValue)
  ...
  textblockFromContext() {
    ...
    if ($context)
      for (let d = $context.depth; d >= 0; d--) {
        const deflt = $context.node(d).contentMatchAt($context.indexAfter(d)).defaultType;
        if (deflt?.isTextblock && deflt.defaultAttrs) return deflt;
      }
...
function normalizeList(dom: DOMNode) {
  for (let child = dom.firstChild, prevItem = null; child; child = child.nextSibling) {
    ...
    if (name && Object.hasOwn(listTags, name) && prevItem) {
...
function parseStyles(style: string): string[] {
  ...
  const styleRegex = /\s*([\w-]+)\s*:\s*([^;]+)/g;

  let match: RegExpExecArray | null;
  // biome-ignore lint/suspicious/noAssignInExpressions: I don't know why they write code like this
  while ((match = styleRegex.exec(style))) {
