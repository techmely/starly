...
export class Node {
  ...
  nodesBetween(from: number, to: number, f: NodeCallbackFn, startPos = 0) {
    this.content.nodesBetween(from, to, f, startPos, this);
  }

  /// Call the given callback for every descendant node. Doesn't
  /// descend into a node when the callback returns `false`.
  descendants(f: NodeCallbackFn) {
  ...
  canReplace(
    ...
    const two = one?.matchFragment(this.content, to);
  ...
  canReplaceWith(from: number, to: number, type: NodeType, marks?: readonly Mark[]) {
    ...
    const end = start?.matchFragment(this.content, to);
  ...
  check() {
    ...
    // biome-ignore lint/complexity/noForEach: this is a func of class
