...
function removeRange(content: Fragment, from: number, to: number): Fragment {
  ...
  if (index !== indexTo || !child) throw new RangeError("Removing non-flat range");
...
function addRange(
  ...
  const node = (($end || $start) as ResolvedPos).node(depth);
  let startIndex = 0;
  ...
  if ($start) {
    ...
    if ($start.depth > depth) {
      startIndex++;
    } else if ($start.textOffset && $start.nodeAfter) {
  ...
  if ($end && $end.depth === depth && $end.textOffset && $end.nodeBefore)
