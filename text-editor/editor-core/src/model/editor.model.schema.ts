...
import { ContentMatch } from "./editor.model.content";
import { Fragment } from "./editor.model.fragment";
import type { ParseRule } from "./editor.model.fromDom";
import { Mark } from "./editor.model.mark";
import { Node, TextNode } from "./editor.model.node";
import type { DOMOutputSpec } from "./editor.model.toDom";
...
export class NodeType {
  ...
  createAndFill(
    ...
    const after = matched?.fillBefore(Fragment.empty, true);
...
export class Schema<Nodes extends string = any, Marks extends string = any> {
  ...
  constructor(spec: SchemaSpec<Nodes, Marks>) {
    this.spec = {} as any;
    const instanceSpec = this.spec;
    // @ts-expect-error Ignore typing
    for (const prop in spec) instanceSpec[prop] = (spec as any)[prop];
